...
class KNRM(torch.nn.Module):
    ...
    def _get_mlp(self) -> torch.nn.Sequential:
        def generate_seq():
            ...
            out_layers = [self.kernel_num] + self.out_layers + [1]
            seq = []
            for i in range(1, len(out_layers)):
                in_dim = out_layers[i-1]
                out_dim = out_layers[i]
                pack = [
                    nn.ReLU(),
                ...
                seq.extend(pack)
            return seq
        return nn.Sequential(*generate_seq())
    ...
    def _get_matching_matrix(self, query: torch.Tensor, doc: torch.Tensor) -> torch.FloatTensor:
        ...
        for n_batch in range(batch_size):
            s_query = query[n_batch]
            # [right, EMB_SIZE]
            # print(s_query.shape)
            query_embs = self.embeddings(s_query)

            s_doc = doc[n_batch]
            # [Left, EMB_SIZE]
            doc_embs = self.embeddings(s_doc)

            query_to_stack = []
            for i in range(len(s_query)):
                query_emb = query_embs[i].reshape(1, -1)
                query_row = cos(query_emb, doc_embs)
                query_to_stack.append(query_row)

            batch_matching_matrix = torch.stack(query_to_stack)
            batch_to_stack.append(batch_matching_matrix)

        matching_matrix = torch.stack(batch_to_stack)
    ...
    def predict(self, inputs: Dict[str, torch.Tensor]) -> torch.FloatTensor:
        ...
        # shape = [Batch, Kernels]
        # matching_matrix = torch.rand_like(matching_matrix, requires_grad = True)

        kernels_out = self._apply_kernels(matching_matrix)
        # print(kernels_out.shape)
        # shape = [Batch]
...
class TrainTripletsDataset(RankingDataset):
    def __getitem__(self, idx):
        q_org, q_cand1, q_cand2, label = self.index_pairs_or_triplets[idx]

        left_elem = {
            'query': self._convert_text_idx_to_token_idxs(q_org),
            'document': self._convert_text_idx_to_token_idxs(q_cand1),
        }
        right_elem = {
            'query': self._convert_text_idx_to_token_idxs(q_org),
            'document': self._convert_text_idx_to_token_idxs(q_cand2),
        }
...
class ValPairsDataset(RankingDataset):
    def __getitem__(self, idx):
        q_org, q_for_validate, label = self.index_pairs_or_triplets[idx]

        left_elem = {
            'query': self._convert_text_idx_to_token_idxs(q_org),
            'document': self._convert_text_idx_to_token_idxs(q_for_validate),
        }
...
class Solution:
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        ...
        tokens = []
        for txt in uniq_txt:
            tokens.extend(self.simple_preproc(txt))
        c = Counter(tokens)
        return self._filter_rare_words(c, min_occurancies)
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        def _generate_new_tensor(rand_uni_bound, shape=(1, 1)):
            return np.random.uniform(-rand_uni_bound, rand_uni_bound, shape).astype(np.float32)

        w_to_emb = self._read_glove_embeddings(file_path)
        ...
        unk_words = ['PAD', 'OOV']
        matrix = [
            torch.zeros(1, EMB_SIZE),
            _generate_new_tensor(rand_uni_bound, shape=(1, EMB_SIZE)),
        ]

        for idx, token in enumerate(inner_keys):
            idx += 2
            if token in w_to_emb:
                emb = w_to_emb[token]
            else:
                emb = _generate_new_tensor(rand_uni_bound, shape=(1, EMB_SIZE))
    ...
    def sample_data_for_train_iter(self, inp_df: pd.DataFrame, seed: int
        ...
        inp_df_group = inp_df.groupby("id_left").size()
        glue_dev_leftids_to_use = list(
            inp_df_group[inp_df_group.between(2, 10)].index)
        selected = inp_df[inp_df.id_left.isin(glue_dev_leftids_to_use)]
        return self.create_train_triplets(selected, fill_top_to=5)[:10000]

    def create_train_triplets(self, inp_df: pd.DataFrame, fill_top_to: int = 15,
        ...
        for id_left, group in groups:
            ...
            relevance = {
                **{id_: 2 for id_ in ones_ids},
                **{id_: 1 for id_ in zeroes_ids},
                **{id_: 0 for id_ in pad_sample},

            }

            ranked = [*ones_ids, *zeroes_ids, *pad_sample]
            for q1, q2 in it.combinations(ranked, 2):
                def compare_relevance(q1, q2):
                    q1_relevance = relevance[q1]
                    q2_relavance = relevance[q2]
                    if q1_relevance > q2_relavance:
                        return 1
                    elif q1_relevance < q2_relavance:
                        return 0
                    else:
                        return 0.5

                out_triples.append(
                    [id_left, q1, q2, compare_relevance(q1, q2)]
                )
    ...
    def valid(self, model: torch.nn.Module, val_dataloader: torch.utils.data.DataLoader) -> float:
        ...
        labels_and_groups = pd.DataFrame(labels_and_groups, columns=[
    ...
    def train(self, n_epochs: int):
        ...
        # train_triplets = self.sample_data_for_train_iter(
        #     self.glue_train_df, seed=0)
        ...
        #     i = 0
        #     # time_to_break = 100
        #     for q1, q2, y_true in train_dataloader:
        #         # print("new batch")
        #         opt.zero_grad()

        #         y_pred = self.model(q1, q2).reshape(-1, 1)

        #         # print(y_true)
        #         # y_pred = y_pred.detach()
        #         # print(y_pred,y_true)
        #         # y_pred = torch.rand_like(batch_pred, requires_grad = True)

        #         # print(y_pred.shape,y_true.shape)
        #         # y_pred.backward(retain_graph=True)
        #         # print("calc loss")
        #         batch_loss = criterion(y_pred.float(), y_true)
        ...
        #         # if i==time_to_break:
        #         #     break
        #     print("@avgloss", np.mean(avg_loss))
        #     # with torch.no_grad():
        #     #     model.eval()
        #     #     ndcg = valid(model, val_dataloader)
        #     # print("@ndcg", ndcg)
